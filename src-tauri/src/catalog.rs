...
///
/// A catalog generated by the Import Scanner (issue #4) is written to the app
/// data directory and takes precedence; when none exists yet — e.g. on a fresh
/// install before the first Vault scan — we fall back to the bundled mock
/// resource so the UI always has something to show.
#[tauri::command]
