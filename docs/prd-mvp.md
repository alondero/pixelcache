...
Pixelcache is a lightweight, cross-platform launcher built with Tauri and React. It groups regional variations, beta builds, and ROM hacks under a single canonical Game card in the UI, enabling a clean and clutter-free interface. For the MVP, Pixelcache operates entirely offline-first, launching games directly from a local directory (Vault) and automatically populating the game library by scanning and parsing ROM filenames.
...
5. As a homelab user, I want to point Pixelcache to my local ROM folder (Vault), so that it automatically scans my files, detects the titles, regions, and platforms from the filenames, and generates my initial catalog without manual configuration.
...
2. **Import Scanner Module (Rust)**
   - Crawls a specified local Vault directory.
   ...
   - Generates the initial Catalog file.
...
- **Release**: `id` (required, string), `gameId` (required, string), `title` (required, string), `region` (optional, string), `platform` (required, string), `revision` (optional, string), `releaseType` (required, enum: retail/beta/hack/translation/homebrew), `publisher` (optional, string), `filePath` (required, string), `media` (optional object with `video` and `image` paths).
